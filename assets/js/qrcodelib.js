...
function Detector(image) {
	...
	this.sizeOfBlackWhiteBlackRun = function (fromX, fromY, toX, toY) {
		...
		for (var x = fromX, y = fromY; x != toX; x += xstep) {
			...
			} else if (!this.image[realX + realY * qrcode.width]) state++;
			if (state == 3) {
				var diffX = x - fromX;
				var diffY = y - fromY;
	...
	this.processFinderPatternInfo = function (info) {
		...
		else points = new Array(bottomLeft, topLeft, topRight, alignmentPattern);
...
var FORMAT_INFO_DECODE_LOOKUP = new Array(
	new Array(21522, 0),
	new Array(20773, 1),
	new Array(24188, 2),
	new Array(23371, 3),
...
FormatInformation.numBitsDiffering = function (a, b) {
	a ^= b;
	return (
		BITS_SET_IN_HALF_BYTE[a & 15] +
		BITS_SET_IN_HALF_BYTE[URShift(a, 4) & 15] +
		BITS_SET_IN_HALF_BYTE[URShift(a, 8) & 15] +
		BITS_SET_IN_HALF_BYTE[URShift(a, 12) & 15] +
		BITS_SET_IN_HALF_BYTE[URShift(a, 16) & 15] +
		BITS_SET_IN_HALF_BYTE[URShift(a, 20) & 15] +
		BITS_SET_IN_HALF_BYTE[URShift(a, 24) & 15] +
		BITS_SET_IN_HALF_BYTE[URShift(a, 28) & 15]
	);
};
...
function BitMatrix(width, height) {
	...
	this.clear = function () {
		...
		for (var i = 0; i < max; i++) this.bits[i] = 0;
	};
	...
	this.unmaskBitMatrix = function (bits, dimension) {
		for (var i = 0; i < dimension; i++)
			for (var j = 0; j < dimension; j++)
				if (this.isMasked(i, j)) bits.flip(j, i);
	};
...
qrcode.decode = function (src) {
	if (arguments.length == 0) {
		var canvas_qr = qrcode.sourceCanvas
			? qrcode.sourceCanvas
...
qrcode.getMiddleBrightnessPerArea = function (image) {
	...
	for (var i = 0; i < numSqrtArea; i++) {
		...
		for (var i2 = 0; i2 < numSqrtArea; i2++) minmax[i][i2] = new Array(0, 0);
	}
	...
	for (var i3 = 0; i3 < numSqrtArea; i3++) middle[i3] = new Array(numSqrtArea);
	for (var ay = 0; ay < numSqrtArea; ay++)
		for (var ax = 0; ax < numSqrtArea; ax++)
...
function FinderPattern(posX, posY, estimatedModuleSize) {
	...
	this.aboutEquals = function (moduleSize, i, j) {
		if (
			...
			return (
				moduleSizeDiff <= 1 || moduleSizeDiff / this.estimatedModuleSize <= 1
			);
		}
		return false;
	};
	...
	this.findFinderPattern = function (image) {
		...
		for (var i = iSkip - 1; i < maxI && !done; i += iSkip) {
			...
			if (this.foundPatternCross(stateCount)) {
				...
				if (confirmed) {
					iSkip = stateCount[0];
					if (this.hasSkipped) done = haveMultiplyConfirmedCenters();
				}
			}
		}
...
function AlignmentPatternFinder(
	...
	this.find = function () {
		...
		for (var iGen = 0; iGen < height; iGen++) {
			var i =
				middleI + ((iGen & 1) == 0 ? (iGen + 1) >> 1 : -((iGen + 1) >> 1));
			stateCount[0] = 0;
			stateCount[1] = 0;
			stateCount[2] = 0;
			var j = startX;
			while (j < maxJ && !image[j + qrcode.width * i]) j++;
			var currentState = 0;
			while (j < maxJ) {
				if (image[j + i * qrcode.width])
